# Archive Collection: validate-signal-model.sh
# Source: signal-model/scripts/validate.sh
...
echo "🌌 Checking constellation aggregation files..."
if [ -f "$MODULE_DIR/scripts/validate-aggregations.sh" ]; then
  if "$MODULE_DIR/scripts/validate-aggregations.sh"; then
    echo "  ✓ Aggregation validation passed"
  else
    echo "  ❌ Aggregation validation failed"
  fi
else
  echo "  ⚪ No aggregation validator found"
fi
